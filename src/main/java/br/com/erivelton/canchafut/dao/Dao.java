...
public class Dao<E> implements Serializable {
	...
	public void removerPorId(Long id, Class<E> entidade) {
		try {
			em.clear();
			EntityTransaction trans = em.getTransaction();
			trans.begin();
			E objeto = em.find(entidade, id);
			em.remove(objeto);
			trans.commit();
		} catch (Exception e) {
			em.getTransaction().rollback();
	...
	public void removerPorRegistro(Class<E> ent, String atributo, Object valor) {
		try {
			em.clear();
			EntityTransaction trans = em.getTransaction();
			trans.begin();
			em.createQuery("delete from " + ent.getSimpleName() + " where " + atributo.toLowerCase() + " = '" + valor + "'").executeUpdate();
			trans.commit();
		} catch (Exception e) {
			em.getTransaction().rollback();
			e.printStackTrace();
