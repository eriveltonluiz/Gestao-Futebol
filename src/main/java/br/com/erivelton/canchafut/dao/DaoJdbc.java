...
import br.com.erivelton.canchafut.conexaoBD.UtilBD;
import br.com.erivelton.canchafut.conexaoBD.UtilBDJdbc;
...
public class DaoJdbc implements Serializable {
	...
	public List<AgendamentoTimes> listarJdbcAg(Agenda agendaEsc) {
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setTipoStatus(TipoStatusHorario.valueOf(rs.getString("tipoStatus")));
	...
	public List<PagMandante> listaHorariosConfirmados(Long id) {
		String sql = "select valorPendente,dataPag,valorPago, null as situacao from timemandante as t "
				...
				+ "where p.valorPendente = 0 and t.cliente_idCliente = " + id + " union "
				...
				+ "where v.valorPendente = 0 and c.idCliente = " + id;
		try {
			PreparedStatement ps = con.prepareStatement(sql);
	...
	public List<PagMandante> listaPendencias(Long id) {
		String sql = "select valorPendente,dataPag,valorPago, null as situacao from timemandante as t "
				+ "inner join pagmandante as p on p.mandante_idMandante = t.idMandante "
				+ "inner join cliente as c on c.idCliente = t.cliente_idCliente "
				+ "where p.valorPendente > 0 and t.cliente_idCliente = " + id + " union "
