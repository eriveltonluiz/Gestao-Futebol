...
import br.com.erivelton.canchafut.dao.controller.DaoControllerMandantesCamp;
...
import br.com.erivelton.canchafut.dao.controller.DaoControllerVisitantesCamp;
...
//				}
				if (timeCamp.getNome().equals(confronto.nomeTimeMandante())) {
					apm.incluirAmareloAoJogadorTimeMandante(j, confronto);
					daoControllerAmarelosMand.atualizar(apm);   /* Criar uma abstração para evitar duplicidade das 2 condições*/
				} else {
					apv.incluirAmareloAoJogadorTimeVisitante(j, confronto);
					daoControllerAmarelosVisit.atualizar(apv);
				}
			}
...
//				}
				...
				if (timeCamp.getNome() == confronto.getMandante().getTime().getNome()) {
					vpm.incluirVermelhoAoJogadorTimeMandante(j, confronto);
					daoControllerVermelhosMand.atualizar(vpm);
				} else {
					vpv.incluirVermelhoAoJogadorTimeVisitante(j, confronto);
					daoControllerVermelhosVisit.atualizar(vpv);
				}
	...
	private void atualizarResultado(String tipo) {
		...
		case "+":
			...
			if (tipo == "mandante") {
				confrontos.get(ind).getMandante().acrescentarGols(jogadoresTarget.size());
